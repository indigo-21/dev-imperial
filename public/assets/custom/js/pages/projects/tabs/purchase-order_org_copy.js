...
function getPurchaseOrderLineItems(){
    ...
    purchase_order_items.each((index, item)=>{
        data["items"] += {
            cost_plan_section_id: $(item).find(`[name='cost_plan_section_id[${index}]']`).val(),
            section_code: $(item).find(`[name='section_code[${index}]']`).val(),
            item_code: $(item).find(`[name='item_code[${index}]']`).val(),
        }
    })
}
